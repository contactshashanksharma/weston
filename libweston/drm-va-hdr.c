...
#ifdef HDR_DEBUG
static void drm_print_fb_desc(struct drm_fb *fb)
{
	weston_log_continue("VA: Tone mapped fb\n");
	weston_log_continue("VA: format:0x%x bpp:%d name:%s num_planes:%d\n",
			    fb->format->format,
			    fb->format->bpp,
...
void drm_print_va_buffer_desc(VADRMPRIMESurfaceDescriptor *vad)
{
	uint32_t i;

	weston_log_continue("===================== VA DESC ====================\n");
	weston_log_continue("VA: va_fourcc:0x%x w:%d h:%d num_o:%d num_lay:%d\n",
				vad->fourcc,
				vad->width,
				vad->height,
				vad->num_objects,
	...
	for (i = 0; i < vad->num_layers; i++) {
		uint32_t j;
		weston_log_continue("VA: layer %d: drm fourcc: 0x%x num_planes %d\n",
					i,
					vad->layers[i].drm_format,
					vad->layers[i].num_planes);

		for (j = 0; j < vad->layers[i].num_planes; j++) {
			weston_log_continue("\tVA:plane %d: offs 0x%x pitc %d index 0x%x\n",
						j,
						vad->layers[i].offset[j],
						vad->layers[i].pitch[j],
			      ...
			      VAHdrMetaData *out_metadata)
{
	...
	o_hdr10_md->white_point_x = t_smd->white_point_x;
	o_hdr10_md->white_point_y = t_smd->white_point_y;
	o_hdr10_md->display_primaries_x[0] = t_smd->primary_g_x;
	o_hdr10_md->display_primaries_x[1] = t_smd->primary_b_x;
	o_hdr10_md->display_primaries_x[2] = t_smd->primary_r_x;
	o_hdr10_md->display_primaries_y[0] = t_smd->primary_g_y;
	o_hdr10_md->display_primaries_y[1] = t_smd->primary_b_y;
	o_hdr10_md->display_primaries_y[2] = t_smd->primary_r_y;
			      ...
			      uint8_t tm_type)
{
	...
	case VA_TONE_MAPPING_HDR_TO_SDR:
		...
		in_hdr10_md->display_primaries_x[0] = color_primary(s->display_primary_b_x);
		in_hdr10_md->display_primaries_y[0] = color_primary(s->display_primary_b_y);
		in_hdr10_md->display_primaries_x[1] = color_primary(s->display_primary_g_x);
		in_hdr10_md->display_primaries_y[1] = color_primary(s->display_primary_g_y);
		in_hdr10_md->display_primaries_x[2] = color_primary(s->display_primary_r_x);
		in_hdr10_md->display_primaries_y[2] = color_primary(s->display_primary_r_y);
		in_hdr10_md->white_point_x = color_primary(s->white_point_x);
		in_hdr10_md->white_point_y = color_primary(s->white_point_y);
		break;
			 ...
			 uint8_t tm_type)
{
	...
	va_status = vaQueryVideoProcFilterCaps(d->va_display,
				context_id,
...
* The caller needs to free the buffer.
*/
struct drm_fb *
		...
		struct drm_tone_map *tm)
{
	...
	if (!d || !fb || !tm) {
		weston_log("VA: NULL input, VA not initialized ?\n");
		return NULL;
	}
...
static VASurfaceID
drm_va_create_surface(struct drm_va_display *d, int width, int height)
{
    	VAStatus va_status;
	VASurfaceID surface_id;
...
static int drm_va_check_entrypoints(struct drm_va_display *d)
{
	...
	/* Query supported Entrypoint for video processing, Check
	* if VPP is supported by driver */
...
static int drm_va_check_attributes(struct drm_va_display *d)
{
	...
	va_status = vaGetConfigAttributes(d->va_display,
					  VAProfileNone,
					  VAEntrypointVideoProc,
					  &d->attrib,
					  1);
...
drm_va_init_display(struct drm_va_display *d)
{
	...
	va_status = vaInitialize(va_display, &d->major_ver, &d->minor_ver);
